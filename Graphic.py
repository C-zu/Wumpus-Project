...
def main():
 ...
 #           path[i] = map_pos(path[i])
    print(path)
    direction_changes = {
    0: {"RIGHT": ("UP", agent_up), "LEFT": ("DOWN", agent_down), "UP": ("LEFT", agent_left), "DOWN": ("RIGHT", agent_right)},
    1: {"RIGHT": ("DOWN", agent_down), "LEFT": ("UP", agent_up), "UP": ("RIGHT", agent_right), "DOWN": ("LEFT", agent_left)},
    2: {"RIGHT": ("LEFT", agent_left), "LEFT": ("RIGHT", agent_right), "UP": ("DOWN", agent_down), "DOWN": ("UP", agent_up)}
    }
    run = True
    agent = agent_right
    agent_dir = "RIGHT"
    ...
    while (run):
        if (path_i < len(path)):
            ...
            if type(path[path_i]) is not tuple:
                if path[path_i].value in list(direction_changes.keys):
                ...
                # if (path[path_i].value == 5):
                #     cave[current_path[0],current_path[1]] = cave[current_path[0],current_path[1]].replace("W","")
